...
function promoteDecisionSnippets(snippetItems = [], existingDecisionItems = [], limit = 3) {
...
export async function buildRecallBundle(query) {
  ...
  const timelineItems = findEventsByEntity(query, initialIntent === "working_memory_continuity" ? 6 : 4);
  let decisionItems = findDecisionsByEntity(query, initialIntent === "working_memory_detail" ? 6 : 3);
  if (looksLikeHistoryQuery(query)) {
    decisionItems = [...decisionItems, ...promoteDecisionSnippets(snippetItems, decisionItems, 3)];
  }
