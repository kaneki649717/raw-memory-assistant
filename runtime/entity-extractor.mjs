...
// 【2026-03-10 扩展】增加更多模型名匹配
const MODEL_RE = /\b(?:custom-1\/gpt-5\.2|duojie\/gpt-5\.4|zai\/glm-4\.7|bailian\/qwen3\.5-plus|BAAI\/bge-m3|gemini-3\.1-pro|gpt-5\.3-codex|qwen3\.5-plus|glm-4\.7|gpt-[\w.-]+|glm-[\w.-]+|qwen[\w.-]*|bge-[\w.-]+|embedding-[\w.-]+)\b/gi;

// 【2026-03-10 扩展】增加更多配置文件匹配
const CONFIG_RE = /\b(?:openclaw\.json|MEMORY\.md|AGENTS\.md|SOUL\.md|USER\.md|IDENTITY\.md|TOOLS\.md|HEARTBEAT\.md|BOOTSTRAP\.md|config\.json|memorySearch|primary|maxTokens|contextWindow|apiKey|baseUrl|embedding|model)\b/g;

// 【2026-03-10 扩展】增加更多命令匹配
const COMMAND_RE = /\b(?:openclaw status|openclaw doctor|openclaw gateway restart|git clone|pnpm install|npm install|python\s+[^\n]+|node\s+[^\n]+)\b/g;

// 【2026-03-10 扩展】增加更多错误类型匹配
